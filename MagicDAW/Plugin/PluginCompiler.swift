...
class PluginCompiler {
    ...
    private func generateAUWrapper(graph: NodeGraphDefinition) -> String {
        ...
        let subType = Self.fourCCLiteral(from: safeName)
        let manufacturer = Self.fourCCLiteral(from: manufacturerCode)
        ...
        @objc class \(safeName)AudioUnitFactory: NSObject, AUAudioUnitFactory {
            ...
            @objc func beginRequest(with context: NSExtensionContext) {
                // No custom extension request handling yet.
            }
        }
        ...
        final class \(safeName)AudioUnit: AUAudioUnit {
            ...
            static let auComponentDescription = AudioComponentDescription(
        ...
        code += "        _outputBusArray = AUAudioUnitBusArray(audioUnit: self, busType: .output, busses: [outputBus])\n"
        ...
        // Render helpers + render block
        code += """
            // MARK: - Render

            private func performRender(timestamp: UnsafePointer<AudioTimeStamp>,
                                       frameCount: AUAudioFrameCount,
                                       outputData: UnsafeMutablePointer<AudioBufferList>?,
                                       renderEvent: UnsafePointer<AURenderEvent>?,
        ...
        if isInstrument {
            code += """
                processRenderEvents(renderEvent)
        ...
        if isInstrument {
            code += """
            private func processRenderEvents(_ renderEvent: UnsafePointer<AURenderEvent>?) {
                var event = renderEvent.map { UnsafeMutablePointer(mutating: $0) }
                while let currentEvent = event {
                    if currentEvent.pointee.head.eventType == AURenderEventType.MIDI {
                        let midi = currentEvent.pointee.MIDI
        ...
        code += """
            ...
            static func registerAUv3() {
                AUAudioUnit.registerSubclass(
                    ...
                    as: Self.auComponentDescription,
                    name: "\(graph.metadata.name)",
                    version: UInt32(1)
                )
            }
    ...
    /// Generate a numeric FourCC literal from a string name.
    static func fourCCLiteral(from name: String) -> String {
        let chars = Array(name.prefix(4).utf8)
        let padded = chars + Array(repeating: UInt8(ascii: " "), count: max(0, 4 - chars.count))
        let value = padded.prefix(4).reduce(UInt32(0)) { partial, byte in

...
function velocityToColor(v: number): string {
  // Map 0-127 velocity to aurora gradient: deep purple -> teal -> cyan -> white
  const t = v / 127;
...
export const LivePianoRoll: React.FC<LivePianoRollProps> = ({
  ...
  const midiMin = octLow * 12 + 12; // C(octLow) in MIDI
  ...
  // Particle seeds for active note particles
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: 'transparent', overflow: 'hidden' }}>
      {/* Use a div-based layout that fills the container */}
      <div style={{ width: '100%', height: '100%', position: 'relative' }}>
        ...
        >
          <defs>
            ...
            <filter id="lpr-note-glass" x="-10%" y="-20%" width="120%" height="140%">
              <feGaussianBlur in="SourceAlpha" stdDeviation="1" result="shadow" />
              <feOffset in="shadow" dx="0" dy="1" result="offsetShadow" />
              <feFlood floodColor="rgba(255,255,255,0.08)" result="glassColor" />
              <feComposite in="glassColor" in2="SourceGraphic" operator="in" result="glassFill" />
              <feMerge>
          ...
          {(() => {
            ...
            const playheadVisible =
              playheadLocalBeat >= 0 && playheadLocalBeat <= totalBeats;
              ...
              <g opacity={gridEntrance}>
                    ...
                    <g key={`key-${i}`}>
                      ...
                      {black && !isActive && (
                        <rect
                          ...
                          width={keyW}
                          height={KEY_H - 0.3}
                          fill="url(#black-key-aurora)"
                          rx={1}
                          opacity={0.5}
                        />
                      )}
                      {/* Active key glow pulse */}
                      {isActive && (
                        <rect
                          x={0}
                          y={y - 1}
                      ...
                      {/* Key labels on C notes */}
                      {isC && (
                        <text
                          x={PIANO_W - 4}
                          y={y + KEY_H * 0.7}
                          textAnchor="end"
                          fill={isActive ? '#ffffff' : aurora.textDim}
                          fontSize={7}
                  ...
                  return (
                    <g key={`active-row-${pitch}`}>
                      ...
                      <rect
                        ...
                        height={KEY_H}
                        fill={aurora.cyan}
                        opacity={0.06 + 0.03 * Math.sin(frame * 0.12)}
                      />
                      {/* Bright edge */}
                      <line
                        x1={ROLL_X}
                ...
                {playheadVisible && isPlaying && (
                  <rect
                    x={ROLL_X}
                    y={0}
                    width={Math.max(0, playheadX - ROLL_X)}
                    height={GRID_H}
                    fill="url(#playhead-trail)"
                    opacity={0.6}
                  />
                )}
                ...
                {visibleNotes.map((note, ni) => {
                  ...
                  const noteX =
                    ROLL_X + (note.start - scrollOffsetBeats) * BEAT_W;
                  ...
                  return (
                    ...
                    >
                      ...
                      <rect
                        ...
                        height={noteH + 2}
                        rx={4}
                        fill={velocityToColor(note.velocity)}
                        opacity={0.1}
                        filter="url(#lpr-glow)"
                      />
                      {/* Note body - glass effect */}
                      <rect
                        x={noteX}
                        y={noteY}
                        width={noteW}
                ...
                {playheadVisible &&
                  ...
                  activeNotes.map((pitch) => {
                    ...
                    const h = KEY_H - 2;
                    // Growing rectangle from some start point to playhead
                    const rectX = playheadX - 8;
                    const rectW = 8;

                    const pulseOpacity =
                      0.5 + 0.2 * Math.sin(frame * 0.15);
                ...
                {/* ── Particles drifting up from active notes ────── */}
                {activeNotes.map((pitch) => {
                  if (pitch < midiMin || pitch >= midiMax) return null;
                  const rowIdx = totalKeys - 1 - (pitch - midiMin);
                  ...
                  return (
                    <g key={`particles-${pitch}`}>
                      {particleSeeds.map((seed, pi) => {
                        const age =
                          ((frame + seed.delay) % seed.life) / seed.life;
                ...
                <g>
                  ...
                  {visibleNotes.map((note, ni) => {
                    ...
                    return (
                      <g key={`vel-${ni}`} opacity={barEntrance}>
                        {/* Bar glow */}
                        <rect
                          x={x}
                          y={GRID_H + 4 + VEL_H - barH * barEntrance}
                          width={5}
                          height={barH * barEntrance}
                          rx={1}
                          fill={color}
                          opacity={0.2}
                          filter="url(#lpr-glow)"
                  ...
                  {/* Smooth envelope curve connecting velocity peaks */}
                  {visibleNotes.length > 1 && (() => {
                    ...
                    const points = visibleNotes
                      ...
                      .map((note) => {
                        const x =
                          ROLL_X +
                          (note.start - scrollOffsetBeats) * BEAT_W +
                          3;
                        const barH = (note.velocity / 127) * maxBarH;

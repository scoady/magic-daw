...
const VUBar: React.FC<{
  ...
  // Spring-based smooth level: rises fast, falls slow
  ...
  // Peak hold — stays near max, slowly decays
  ...
  // Ghost trail — fading recent peak history
    ...
    <svg width={width} height={height} style={{ filter: blazeFilter }}>
      ...
      {peakDecay > 0.02 && (
        <rect
          ...
          width={width}
          height={2}
          fill={peakDecay > 0.85 ? AURORA.pink : AURORA.gold}
          opacity={0.95}
        />
      )}
    </svg>
...
const FaderVis: React.FC<{
  ...
  // Subtle particle effect near knob
...
const PanArc: React.FC<{
  ...
  // Full arc = center (pan=0), half arc on one side for hard pan
...
const EqCurve: React.FC<{
  ...
  // Subtle pulse animation on the curve
    ...
    >
      ...
      {isMuted && (
        <div style={{
          ...
          <svg width={40} height={40} opacity={0.3}>
            <line x1={5} y1={5} x2={35} y2={35} stroke={AURORA.pink} strokeWidth={2} />
            <line x1={35} y1={5} x2={5} y2={35} stroke={AURORA.pink} strokeWidth={2} />
          </svg>
        </div>
      )}

      {/* Track name + color dot */}
      <div style={{
        display: 'flex',
        alignItems: 'center',
        gap: 4,
        ...
        <div style={{
          ...
          boxShadow: `0 0 6px ${hexToRgba(track.color, 0.6)}`,
        }} />
        <span style={{
          fontSize: 9,
          fontWeight: 600,
      ...
      {/* Pan indicator */}
      <PanArc pan={track.pan} size={28} color={track.color} />
      ...
      <div style={{
        ...
        {/* VU meters — hero element */}
        <VUBar
          level={isMuted ? 0 : levelL}
          width={6}
          height={vuHeight}
          frame={frame}
          fps={fps}
      ...
      <div style={{
        ...
        <div
          ...
          style={{
            ...
            color: isMuted ? '#ef4444' : AURORA.textMuted,
            cursor: 'pointer',
          }}
        >
          M
        ...
        <div
          ...
          style={{
            ...
            color: isSoloed ? AURORA.gold : AURORA.textMuted,
            cursor: 'pointer',
          }}
        >
          S
...
const MasterChannel: React.FC<{
  ...
  // Background aurora wash intensity responds to loudness
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          <VUBar level={levelL} width={8} height={vuHeight} frame={frame} fps={fps}
            channelIndex={100} side="L" />
          <span style={{ fontSize: 6, color: AURORA.textMuted }}>L</span>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          <VUBar level={levelR} width={8} height={vuHeight} frame={frame} fps={fps}
            channelIndex={101} side="R" />
      ...
      <div style={{
        ...
        <div style={{ marginTop: 3, height: 4, background: 'rgba(0,0,0,0.3)', borderRadius: 2 }}>
          <div style={{
            height: '100%',
            width: `${Math.max(0, Math.min(100, lufsNorm * 100))}%`,
...
const SpectrumBar: React.FC<{
  ...
  masterLevelL: number;
  masterLevelR: number;
}> = ({ frame, fps, width, height, masterLevelL, masterLevelR }) => {
  const barCount = 64;
  ...
  return (
    <svg width={width} height={height}>
      ...
      {Array.from({ length: barCount }, (_, i) => {
        ...
        // Simulate spectrum content — bass heavy, treble rolloff
...
const ConstellationGrid: React.FC<{
  ...
  return (
    ...
    >
      ...
      {/* Light rays behind active channels */}
    ...
    >
        ...
        {tracks.map((track, i) => {
          ...
          return (
            <ChannelStrip
              ...
              stripHeight={channelAreaHeight - 12}
            />
          );
        })}

        {/* Master channel — rightmost, wider */}
        <MasterChannel
          levelL={masterLevelL}
          levelR={masterLevelR}
          frame={frame}
          fps={fps}
          stripWidth={masterWidth}
          stripHeight={channelAreaHeight - 12}
        />
      </div>
      ...
      <div style={{
        ...
        <SpectrumBar
          ...
          width={compWidth}
          height={spectrumHeight}
          masterLevelL={masterLevelL}
          masterLevelR={masterLevelR}
        />
      </div>

      {/* Send routing beams — thin aurora lines between channels with sends */}
      {/* (Rendered only when track type is 'bus', connecting to that bus) */}
      <svg
        width={compWidth}
        height={compHeight}
        style={{ position: 'absolute', top: 0, left: 0, pointerEvents: 'none' }}
      >
        ...
        {tracks.map((track, i) => {
          ...
          // Draw send beams from other tracks to this bus
          const busX = 8 + (i + 0.5) * (stripWidth + 4);
          return tracks.map((src, si) => {
            if (src.id === track.id || src.type === 'bus') return null;
            const srcX = 8 + (si + 0.5) * (stripWidth + 4);
            const beamY = channelAreaHeight * 0.7;
            const beamOpacity = 0.08 + Math.sin(frame * 0.08 + si) * 0.04;
            return (
              <line
